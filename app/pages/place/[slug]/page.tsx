...
async function Article({params}:{params:{slug:string}}) {
  ...
  return (
    ...
    <div className='pt-[5rem] px-5 flex flex-col items-center justify-center gap-10 bg-sky-950  '>
        ...
        <Image priority placeholder='blur' blurDataURL={urlFor(image!).url()} className=' rounded-xl w-full lg:w-[50%] h-1/2 flex items-center object-cover' src={urlFor(image!).url()} height={500} width={500}  alt={title}/>
